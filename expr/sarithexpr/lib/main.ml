...
| True -> BoolT
  ...
  | Not e -> (match(typecheck e) with 
    BoolT ->BoolT  
    | _ -> raise(TypeError "Il not può essere applicato solo su un booleano"))
  | And (e1, e2) -> (match(typecheck e1, typecheck e2) with 
    BoolT,BoolT ->BoolT 
  | _ -> raise(TypeError "Errore sulla and"))
      
  | Or (e1, e2) -> (match(typecheck e1, typecheck e2) with 
  BoolT,BoolT ->BoolT 
| _ -> raise(TypeError "Errore sulla or"))
  ...
  | IsZero e0 -> (match(typecheck e0) with 
    NatT ->BoolT  
    | _ -> raise(TypeError "IsZero non può essere bool"))
